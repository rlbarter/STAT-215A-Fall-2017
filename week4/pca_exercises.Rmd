...
First, the following function defines a ($p \times p$) covariance matrix with decaying off diagonals
...
**Let $n = 10000$. Use Cholesky decomposition to generate a dataset (an $n \times p$ matrix $X$) which has covariance matrix `m_sparse` (compare the computation time between `m_sparse` and `m_dense`). Check that the covariance is correct. Generate a plot showing that adjacent columns are correlated with one another.**
